...
export class AuditService {
  ...
  geteventDetails(data_events: any[]) {
    return data_events.map((event) => {
      return {
        timestamp: new Date(event.returnValues.timestamp * 1000),
        documentId: event.returnValues.documentId,
        documentHash: event.returnValues.documentHash
      }
    });
  }
  ...
  async getEvents(address: any, dateFrom: Date) {
    ...
    return this.geteventDetails(events);
  }

  async addEstablishment(establishmentID: string, address: string): Promise<AuditResponse> {
    if (!util.isValidAddress(address)) {
      await this.toastService.presentToast("Address invalida. ", 'danger');
      return {result: false, data: null} as AuditResponse;
    }

    if (this.wallet.balance === "0") {
      await this.toastService.presentToast("No tiene Suficiente Balance", 'danger');
      return {result: false, data: null} as AuditResponse;
    }
